...
def gen_md_from_matlab_src(file):
    ...
    for index, item in enumerate(fun_line):
        ...
        if not src_content == []:
            ...
            for id, value in enumerate(src_content):
                value = value.replace(' ','')
                value = value.replace('\n','')
                if value.lower() == '##end':
                    src_content = src_content[0:id]
                    break
            md[fun_name[index]] = src_content
...
if __name__ == "__main__":
    ...
    gen_md_path = os.path.join(proj_dir, "docs")
    ...
    for fil_ii in src_files:
        ...
        for fun_name, fun_md in md_dict.items():
            ...
            fil_md_gen = os.path.join(gen_md_path, filName) + '.md'
    ...
    # index.md
    index_md_gen = os.path.join(gen_md_path, 'index.md') 
    with open(index_md_gen, 'w', encoding='utf-8') as file:
        file.writelines('# Welcome to matlab-tools\n')
        file.writelines('For matlab pcode, visit [matlab-tools](https://github.com/xie-dd/matlab-tools).\n')
        file.writelines('## Notes\n')
        file.writelines('- Source code is not provided, only pcode is available\n')
        file.writelines('- Matlab versions should > R2022\n')
        file.writelines('## Function list\n')
        file.writelines('|Function | Description |\n')
        file.writelines('|---      |  ---        |\n')
        for ii in src_files:
            tmp = ii.replace('src', 'docs');
            fileName = tmp.replace('.m', '.md');
            with open(fileName, 'r', encoding='utf-8') as fr:
                content = fr.read()
            res = re.finditer('##',content)
            pos = [item.start() for item in res]
            if len(pos) < 2:
                descr = []
            else:
                descr = content[pos[0]:pos[1]]
            file.writelines(f'|{os.path.basename(ii)}   | {descr}|\n')
    print(f"gen doc: {index_md_gen}")